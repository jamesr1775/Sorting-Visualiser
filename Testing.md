...
6. As a potential user, I would like to pause and start the algorithm.
    - The user can pause and restart the sorting animations by clicking the start / pause button.
7. As a potential user, I would like to take test my knowledge and understanding of the algorithm.
    ...
    - The quiz provide feedback to the user as to whether or not they chose the correct answer.
...
8. As a potential user, I would like to learn things I did not know about algorithms before, and see answers to questions about time and space complexity.
    ...
    - Some of the questions are a little more like general knowledge / trivia to provide more learning of the history and background behind the algorithms.

### Manual Testing
* All manual tests below:
    - were ran on chrome and firefox.
    - were repeated on various screen resolutions that include desktops, laptops, tablets and smartphones using the chrome and firefox developer tools that include desktops, ipad, ipad pro, iphone X, 6 ,7 , 8 and the plus models.
    - were repeated on the developers own smartphone (samsung) and tablet (ipad).

#### Home Page Header
1. The Header responsiveness was tested by varying the screen size to see that the icon image remained centered and undistorted.
2. The logo was tested that when it is pressed it returns / refreshes to the home page and that the tutorial modal appears.
3. The header as the same across all platforms so there is no device specific testing.

#### Tutorial Modal
1. Upon visiting or refreshing the page,  a tutorial carousel modal is presented.
2. The 5 gifs and explanations that describe the controls and the bar chart load and cycle.
3. The carousel navigation works and can be used to go through the tutorial steps.
4. The close button closes the modal.
5. The modal tutorial can be reopened by clicking the question mark on the top right of the chart.
6. When the modal closes, an indicator points to the question mark button , to show thats where the tutorial is to reopen it. 
    <div><br/></div>
    <div align="center">
...
1. Upon visiting the webpage, a bar chart is generated with 10 bars. 
2. Across all platforms the width to fit the numbers of bars should be adjusted to fit all of the bars on one horizontal row.
3. There are two ways to generate a bar chart:
...
4. The responsiveness of the chart has been tested on various screen sizes. An example of the testing is below. 
...
1. The default speed of the algorithm is slow enough so its easier to understand whats currently being sorted.
2. Sliding the slider to the left slows down the animation speed.
3. Sliding the slider to the right speeds up the animation speed.
    <div><br/></div>
    <div align="center">
    <img src="/assets/images/algo-speed.gif" alt="Testing Bar Chart">
    </div>
##### Algorithm Starting and Pausing
1. If an algorithm is not currently selected a modal popup appears to notify the user. The modal shows the user what button to use to select an algorithm
2. When an algorithm is selected the start sorting button starts the sorting and visualisation animations. The button then changes to a pause button.
3. While a sorting algorithm is running the pause button is present and pauses the current sorting if clicked.
4. Changing the speed slider while paused and starting the algorithm again does increase/ decrease the animations speed.
    <div><br/></div>
    <div align="center">
...
3. The font size of the code adjusts for smaller screen sizes so that the user don't have to click into the box and move around the code to view it as much.
##### Selecting an Algorithm
1. The Pick Algorithm drop down menu provides three algorithms to choose from.
2. When an algorithm is selected, the code and the algorithm information are changed from hidden to visible for the chosen algorithm.
...
3. The correct algorithm runs and can visually be identified when the start sorting button is pressed.
4. Swapping between algorithms many times behaves as expected.

##### Bubble Sort
1. The code presented to user is correct and implements the algorithm.
2. This code is tested in jasmine testing also.
3. Bubble sort visually moves the largest bar to the right and starts over at the beginning going to the end which is left the last sorted bar.
    <div><br/></div>
...
1. The code presented to user is correct and implements the algorithm.
2. This code is tested in jasmine testing also.
3. Merge Sort visually uses the divide and conquer approach working on smaller chunks of the array. 
    - It creates works on smaller arrays sorting them 
...
1. The code presented to user is correct and implements the algorithm.
2. This code is tested in jasmine testing also.
3. Quick Sort visually uses the divide and conquer approach also working on smaller chunks of the array. 
...
2. For smartphones, they are displayed on separate rows.
...
1. Quiz Navigation
    ...
    - The take quiz button opens up the drop down tab that contains the quiz. A question alongside multiple choices are presented to the user.
    - Confirmed the correct questions and choices are loaded for each algorithm.
...
##### Quiz Completion
1. Quiz results Modal
    - Upon clicking the final question a modal that presents the results appears to the user. It displays the questions and the result of the users inputs.
...
2. Retake Quiz
    ...
    - The answer explanation, retake quiz and view results buttons all disappear.
    <div><br/></div>
    <div align="center">
...
#### Footer
1. The footer contains the credit information along with a github icon.
2. The github icon opens up the source code for the project into a new tab.
